...
; wait_for_flash_status_done();
       jsr       _wait_for_flash_status_done
       movem.l   (A7)+,D2/D3/A2
       unlk      A6
       rts
; }
; int read_256k_bytes(void){
       xdef      _read_256k_bytes
_read_256k_bytes:
       link      A6,#-8
...
; void load_sdram_from_spi_flash(void){
       xdef      _load_sdram_from_spi_flash
_load_sdram_from_spi_flash:
       link      A6,#-16
       movem.l   D2/A2,-(A7)
       lea       _WriteSPIChar.L,A2
; int i;
...
; for (i=0;i<262144;i++){
       clr.l     D2
load_sdram_from_spi_flash_1:
       cmp.l     #262144,D2
       bge.s     load_sdram_from_spi_flash_3
; current_address = dram_start_address + i; 
       move.l    -4(A6),D0
       add.l     D2,D0
       move.l    D0,-8(A6)
...
; *current_address =  data;
       ...
       move.b    D0,(A0)
       addq.l    #1,D2
       bra       load_sdram_from_spi_flash_1
load_sdram_from_spi_flash_3:
...
; load_sdram_from_spi_flash(); 
       jsr       _load_sdram_from_spi_flash
       rts
; //
...
; void ask_addr_range_sram (unsigned int* addr_array, int data_length) {
       xdef      _ask_addr_range_sram
_ask_addr_range_sram:
       link      A6,#0
       movem.l   D2/D3/D4/A2,-(A7)
       lea       _printf.L,A2
       move.l    8(A6),D2
...
ask_addr_range_sram_1:
       tst.l     D4
       bne       ask_addr_range_sram_3
...
; if (data_length > 1 && addr_array[0] % 2 != 0) {
       ...
       addq.w    #8,A7
       tst.l     D0
       beq.s     ask_addr_range_sram_8
; printf("The start address provided is odd, need an even one!\n");
       pea       @m68kde~1_143.L
       jsr       (A2)
       addq.w    #4,A7
       bra.s     ask_addr_range_sram_9
ask_addr_range_sram_8:
; } else {
...
; start_addr_valid = 1;
       moveq     #1,D4
ask_addr_range_sram_9:
       bra       ask_addr_range_sram_1
ask_addr_range_sram_3:
; } 
; }
; }
; while (!end_addr_valid) {
ask_addr_range_sram_10:
       tst.l     D3
       bne       ask_addr_range_sram_12
...
; if (addr_array[1] < 4026531840) {
       ...
       move.l    4(A0),D0
       cmp.l     #-268435456,D0
       bhs.s     ask_addr_range_sram_13
; printf ("The end address is smaller than 0xf0000000, invalid!\n");
       pea       @m68kde~1_146.L
       jsr       (A2)
       addq.w    #4,A7
       bra       ask_addr_range_sram_18
ask_addr_range_sram_13:
; } else if (addr_array[1] > 4026793983)
       move.l    D2,A0
       move.l    4(A0),D0
       cmp.l     #-268173313,D0
       bls.s     ask_addr_range_sram_15
; {
; printf ("The end address is bigger than 0xf003ffff, invalid!\n");
       ...
       addq.w    #4,A7
       bra       ask_addr_range_sram_18
ask_addr_range_sram_15:
; } else { 
; if (data_length > 1 && addr_array[1] % 2 != 0) {
       move.l    12(A6),D0
       cmp.l     #1,D0
       ble.s     ask_addr_range_sram_17
       move.l    D2,A0
       move.l    4(A0),-(A7)
       pea       2
       jsr       ULDIV
       ...
       addq.w    #8,A7
       tst.l     D0
       beq.s     ask_addr_range_sram_17
; printf("The end address provided is odd, need an even one!\n");
       pea       @m68kde~1_148.L
       jsr       (A2)
       addq.w    #4,A7
       bra.s     ask_addr_range_sram_18
ask_addr_range_sram_17:
; } else {
...
; end_addr_valid = 1;
       moveq     #1,D3
ask_addr_range_sram_18:
       bra       ask_addr_range_sram_10
ask_addr_range_sram_12:
       movem.l   (A7)+,D2/D3/D4/A2
       unlk      A6
       rts
; } 
...
; void ask_addr_range_sdram(unsigned int* addr_array, int data_length) {
       xdef      _ask_addr_range_sdram
_ask_addr_range_sdram:
       link      A6,#0
       movem.l   D2/D3/D4/A2,-(A7)
       lea       _printf.L,A2
       move.l    8(A6),D2
...
; int end_addr_valid = 0;
       clr.l     D3
; while (!start_addr_valid) {
ask_addr_range_sdram_1:
       tst.l     D4
       bne       ask_addr_range_sdram_3
; printf("\r\nProvide the start address of the test.\n");
       pea       @m68kde~1_140.L
       jsr       (A2)
       addq.w    #4,A7
...
; if (addr_array[0] < 134217728) {
       ...
       move.l    (A0),D0
       cmp.l     #134217728,D0
       bhs.s     ask_addr_range_sdram_4
; printf ("The start address is smaller than 0x08000000, invalid!\n");
       pea       @m68kde~1_150.L
       jsr       (A2)
       addq.w    #4,A7
       bra       ask_addr_range_sdram_9
ask_addr_range_sdram_4:
; } else if (addr_array[0] > 201326591) {
       move.l    D2,A0
       move.l    (A0),D0
       cmp.l     #201326591,D0
       bls.s     ask_addr_range_sdram_6
; printf ("The start address is bigger than 0x0bffffff, invalid!\n");
       pea       @m68kde~1_151.L
       jsr       (A2)
       addq.w    #4,A7
       bra       ask_addr_range_sdram_9
ask_addr_range_sdram_6:
; } else { // If the data length is words or long words, check whether the start address is odd 
; if (data_length > 1 && addr_array[0] % 2 != 0) {
       move.l    12(A6),D0
       cmp.l     #1,D0
       ble.s     ask_addr_range_sdram_8
       move.l    D2,A0
       move.l    (A0),-(A7)
       pea       2
       jsr       ULDIV
       ...
       addq.w    #8,A7
       tst.l     D0
       beq.s     ask_addr_range_sdram_8
; printf("The start address provided is odd, need an even one!\n");
       pea       @m68kde~1_143.L
       jsr       (A2)
       addq.w    #4,A7
       bra.s     ask_addr_range_sdram_9
ask_addr_range_sdram_8:
; } else {
; printf ("Start address valid.\n");
       pea       @m68kde~1_144.L
       jsr       (A2)
...
; start_addr_valid = 1;
       moveq     #1,D4
ask_addr_range_sdram_9:
       bra       ask_addr_range_sdram_1
ask_addr_range_sdram_3:
; } 
; }
; }
; while (!end_addr_valid) {
ask_addr_range_sdram_10:
       tst.l     D3
       bne       ask_addr_range_sdram_12
; printf("\r\nProvide the end address of the test.\n");
       pea       @m68kde~1_145.L
       jsr       (A2)
       addq.w    #4,A7
...
; if (addr_array[1] < 134217728) {
       ...
       move.l    4(A0),D0
       cmp.l     #134217728,D0
       bhs.s     ask_addr_range_sdram_13
; printf ("The end address is smaller than 0x08000000, invalid!\n");
       pea       @m68kde~1_152.L
       jsr       (A2)
       addq.w    #4,A7
       bra       ask_addr_range_sdram_18
ask_addr_range_sdram_13:
; } else if (addr_array[1] > 201326591)
       move.l    D2,A0
       move.l    4(A0),D0
       cmp.l     #201326591,D0
       bls.s     ask_addr_range_sdram_15
; {
; printf ("The end address is bigger than 0x0bffffff, invalid!\n");
       ...
       addq.w    #4,A7
       bra       ask_addr_range_sdram_18
ask_addr_range_sdram_15:
; } else { 
; if (data_length > 1 && addr_array[1] % 2 != 0) {
       move.l    12(A6),D0
       cmp.l     #1,D0
       ble.s     ask_addr_range_sdram_17
       move.l    D2,A0
       move.l    4(A0),-(A7)
       pea       2
       jsr       ULDIV
       ...
       addq.w    #8,A7
       tst.l     D0
       beq.s     ask_addr_range_sdram_17
; printf("The end address provided is odd, need an even one!\n");
       pea       @m68kde~1_148.L
       jsr       (A2)
       addq.w    #4,A7
       bra.s     ask_addr_range_sdram_18
ask_addr_range_sdram_17:
; } else {
; printf ("End address valid.\n");
       pea       @m68kde~1_149.L
       jsr       (A2)
...
; end_addr_valid = 1;
       moveq     #1,D3
ask_addr_range_sdram_18:
       bra       ask_addr_range_sdram_10
ask_addr_range_sdram_12:
       movem.l   (A7)+,D2/D3/D4/A2
       unlk      A6
       rts
; } 
...
@m68kde~1_22:
       dc.b      13,10,68,97,116,97,32,100,105,115,99,114,101
       dc.b      112,97,110,99,121,32,97,116,32,108,111,99,97
       dc.b      116,105,111,110,58,32,37,120,32,83,68,82,65
       dc.b      77,32,100,97,116,97,58,32,37,120,32,70,108,97
       dc.b      115,104,32,100,97,116,97,58,32,37,120,0
@m68kde~1_23:
       dc.b      13,10,108,111,99,97,116,105,111,110,58,32,37
       dc.b      120,32,83,68,82,65,77,32,100,97,116,97,58,32
       dc.b      37,120,32,70,108,97,115,104,32,100,97,116,97
       dc.b      58,32,37,120,0
@m68kde~1_24:
       dc.b      13,10,69,114,97,115,105,110,103,32,83,80,73
       dc.b      32,102,108,97,115,104,46,46,46,46,0
@m68kde~1_25:
       dc.b      13,10,87,114,105,116,116,105,110,103,32,116
       dc.b      104,101,32,83,80,73,32,102,108,97,115,104,32
       dc.b      117,115,105,110,103,32,83,68,82,65,77,32,100
       dc.b      97,116,97,46,46,46,46,0
@m68kde~1_26:
       dc.b      13,10,82,101,97,100,105,110,103,32,116,104,101
       dc.b      32,83,80,73,32,102,108,97,115,104,32,100,97
       dc.b      116,97,32,97,110,100,32,99,111,109,112,97,114
       dc.b      105,110,103,32,116,111,32,83,68,82,65,77,46
       dc.b      46,46,46,0
@m68kde~1_27:
...
@m68kde~1_141:
       ...
       dc.b      108,101,114,32,116,104,97,110,32,48,120,102
       dc.b      48,48,48,48,48,48,48,44,32,105,110,118,97,108
       dc.b      105,100,33,10,0
@m68kde~1_142:
       dc.b      84,104,101,32,115,116,97,114,116,32,97,100,100
       dc.b      114,101,115,115,32,105,115,32,98,105,103,103
       dc.b      101,114,32,116,104,97,110,32,48,120,102,48,48
       dc.b      51,102,102,102,102,44,32,105,110,118,97,108
       dc.b      105,100,33,10,0
@m68kde~1_143:
       dc.b      84,104,101,32,115,116,97,114,116,32,97,100,100
...
@m68kde~1_146:
       ...
       dc.b      101,114,32,116,104,97,110,32,48,120,102,48,48
       dc.b      48,48,48,48,48,44,32,105,110,118,97,108,105
       dc.b      100,33,10,0
@m68kde~1_147:
       dc.b      84,104,101,32,101,110,100,32,97,100,100,114
       dc.b      101,115,115,32,105,115,32,98,105,103,103,101
       dc.b      114,32,116,104,97,110,32,48,120,102,48,48,51
       dc.b      102,102,102,102,44,32,105,110,118,97,108,105
       dc.b      100,33,10,0
@m68kde~1_148:
       dc.b      84,104,101,32,101,110,100,32,97,100,100,114
...
@m68kde~1_150:
       ...
       dc.b      48,48,48,48,48,48,44,32,105,110,118,97,108,105
       dc.b      100,33,10,0
@m68kde~1_151:
       dc.b      84,104,101,32,115,116,97,114,116,32,97,100,100
       dc.b      114,101,115,115,32,105,115,32,98,105,103,103
       dc.b      101,114,32,116,104,97,110,32,48,120,48,98,102
       dc.b      102,102,102,102,102,44,32,105,110,118,97,108
       dc.b      105,100,33,10,0
@m68kde~1_152:
       dc.b      84,104,101,32,101,110,100,32,97,100,100,114
       dc.b      101,115,115,32,105,115,32,115,109,97,108,108
...
@m68kde~1_153:
       dc.b      84,104,101,32,101,110,100,32,97,100,100,114
       dc.b      101,115,115,32,105,115,32,98,105,103,103,101
       dc.b      114,32,116,104,97,110,32,48,120,48,98,102,102
       dc.b      102,102,102,102,44,32,105,110,118,97,108,105
       dc.b      100,33,10,0
@m68kde~1_154:
...
@m68kde~1_158:
       dc.b      13,10,68,111,32,121,111,117,32,119,97,110,116
       dc.b      32,116,111,32,116,101,115,116,32,49,46,32,83
       dc.b      82,65,77,32,111,114,32,50,46,32,83,68,82,65
       dc.b      77,63,32,80,114,111,118,105,100,101,32,116,104
       dc.b      101,32,105,110,116,101,103,101,114,32,98,101
       dc.b      108,111,119,46,10,0
@m68kde~1_159:
       dc.b      83,116,97,114,116,32,97,100,100,114,101,115
